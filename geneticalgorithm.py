...
) -> List[Union[np.ndarray, NeuralNetwork]]:
...
def fitness(
    chromosome: Union[np.ndarray, NeuralNetwork], 
    env: gym.Env, 
    model_type: str = 'linear'
) -> float:
    state, _ = env.reset()
...
def selection(
    population: List[Union[np.ndarray, NeuralNetwork]], 
    fitnesses: List[float], 
    elite_size: int
) -> List[Union[np.ndarray, NeuralNetwork]]:
    min_fitness: float = min(fitnesses)
    offset: float = abs(min_fitness) + 1 if min_fitness < 0 else 0
    adjusted_fitnesses: List[float] = [f + offset for f in fitnesses]
    selected: List[Union[np.ndarray, NeuralNetwork]] = random.choices(population, weights=adjusted_fitnesses, k=len(population) - elite_size)
    return selected

def crossover(
    parent1: Union[np.ndarray, NeuralNetwork], 
    parent2: Union[np.ndarray, NeuralNetwork], 
    crossover_rate: float, 
    state_size: int, 
    model_type: str = 'linear'
) -> Tuple[Union[np.ndarray, NeuralNetwork], Union[np.ndarray, NeuralNetwork]]:
    if model_type == 'linear':
        if random.random() < crossover_rate:
            point: int = random.randint(1, state_size - 1)
            child1: np.ndarray = np.vstack((parent1[:point], parent2[point:]))
            child2: np.ndarray = np.vstack((parent2[:point], parent1[point:]))
            return child1, child2
        else:
            return parent1, parent2
    else:
        child1: NeuralNetwork = parent1.copy()
        child2: NeuralNetwork = parent2.copy()
        for key in parent1.__dict__.keys():
            if random.random() < crossover_rate:
                point: int = random.randint(1, getattr(parent1, key).shape[0] - 1)
...
def mutate(
    ...
    action_size: int, 
    model_type: str = 'linear'
) -> Union[np.ndarray, NeuralNetwork]:
    if model_type == 'linear':
        if random.random() < mutation_rate:
            index: int = random.randint(0, state_size - 1)
            chromosome[index] = np.random.randn(action_size)
    else:
        for key in chromosome.__dict__.keys():
            if random.random() < mutation_rate:
                index: int = random.randint(0, getattr(chromosome, key).shape[0] - 1)
